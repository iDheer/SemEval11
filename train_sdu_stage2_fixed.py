"""
Stage 2: Apply SDU (FIXED VERSION WITH PROPER GRADIENTS)
Two-phase approach: Fine-tuning + SDU corrections
"""
...
print("="*80)
print("STAGE 2: SDU APPLICATION (FIXED VERSION WITH GRADIENTS)")
print("="*80)
...
if not os.path.exists("./shadow_model_qwen_final"):
    print("❌ ERROR: Shadow model not found!")
    exit(1)
...
# ============================================================================
# STEP 1: Load Shadow Model and Extract Bias Direction
# ============================================================================
print("\n" + "="*80)
print("STEP 1: Loading Shadow Model")
print("="*80)
...
shadow_model = PeftModel.from_pretrained(base_model, "./shadow_model_qwen_final")
shadow_model.eval()
print("✓ Shadow model loaded")

# ============================================================================
# STEP 2: Extract Bias Direction (VP vs VI)
# ============================================================================
print("\n" + "="*80)
print("STEP 2: Computing Bias Direction")
print("="*80)
...
# ============================================================================
# STEP 3: Load S-CoT Model
# ============================================================================
print("\n" + "="*80)
print("STEP 3: Loading S-CoT Model")
print("="*80)
...
# ============================================================================
# STEP 5: Two-Phase Training (FIXED VERSION)
# ============================================================================
print("\n" + "="*80)
print("STEP 5: Two-Phase SDU Application")
print("="*80)

print("\nPhase 1: Fine-tuning (gradients will flow properly)")
print("Phase 2: SDU corrections (direct weight modification)")
print("="*80)
...
if train_dataset:
    ...
    print("Expected: grad_norm > 0, loss decreases")
    print("="*80 + "\n")
...
# ============================================================================
# PHASE 2: Apply SDU Corrections
# ============================================================================
print("\n" + "="*80)
print("PHASE 2: APPLYING SDU CORRECTIONS")
print("="*80)

print("\nComputing SDU adjustment matrices...")
...
for layer_idx in TARGET_LAYERS:
    ...
    try:
        layer = sdu_model.base_model.model.model.layers[layer_idx]
    except AttributeError:
        print(f"  ⚠ Could not access layer {layer_idx}")
        continue
    
    dz = delta_z[layer_idx].float().cpu()
    
    for name, param in layer.named_parameters():
        if 'lora' not in name.lower() or 'o_proj' not in name:
            continue
        
        W = param.detach().float().cpu()
        
        if W.dim() == 2:
            # Compute delta_K based on weight dimensions
            ...
            adjustment_matrices[layer_idx][full_name] = delta_K

total_params = sum(len(adj) for adj in adjustment_matrices.values())
print(f"✓ Pre-computed adjustments for {total_params} parameters")

# Apply corrections with multiple strengths
CORRECTION_STRENGTHS = [0.5, 1.0, 2.0]

print(f"\nApplying SDU corrections with {len(CORRECTION_STRENGTHS)} different strengths...")
    ...
    with torch.no_grad():
        for layer_idx in TARGET_LAYERS:
            ...
            for name, param in layer.named_parameters():
                ...
                if full_name in adjustment_matrices[layer_idx]:
                    delta_K = adjustment_matrices[layer_idx][full_name].to(param.device)
                    
                    # Apply correction
                    if delta_K.shape == param.shape:
                        correction = delta_K * strength
                        param.data += correction
                        corrections_applied += 1
                        total_correction_norm += correction.norm().item()
                    elif delta_K.numel() == param.numel():
                        correction = delta_K.view_as(param) * strength
                        param.data += correction
                        corrections_applied += 1
                        total_correction_norm += correction.norm().item()
    
    avg_correction = total_correction_norm / max(corrections_applied, 1)
    print(f"  Strength {strength:3.1f}: {corrections_applied} corrections, avg norm: {avg_correction:.6f}")
...
# ============================================================================
# STEP 6: Save SDU Model
# ============================================================================
print("\n" + "="*80)
print("STEP 6: Saving SDU Model")
print("="*80)
...
metadata = {
    ...
    'method': 'Two-phase: Fine-tuning + SDU corrections',
...
print(f"  Phase 2 (SDU): {len(CORRECTION_STRENGTHS)} correction strengths applied")
print(f"  Total SDU corrections: {sum(s['corrections'] for s in correction_stats)}")

print("\nNext: python evaluate_final.py")
print("\nExpected improvements:")
print("  - VI (Valid-Implausible) accuracy may increase")
print("  - Content Effect should stay near zero")
print("  - Overall accuracy: 89-92%")
print("="*80)